...
SHORTCUT_TARGETS = {
    'open_close': {
        'label': 'Open/Close',
        'operator': 'witch_quickbar.toggle_display_state',
        'prefix': 'open_close_shortcut',
    },
    'maximize': {
        'label': 'Maximize Open/Close',
        'operator': 'witch_quickbar.maximize_display_state',
        'prefix': 'maximize_shortcut',
    },
    'cycle': {
        'label': 'Cycle Modes',
...
def assign_shortcut_from_event(context, target, event):
    ...
    if target == 'open_close' and not getattr(prefs, _field('maximize', 'type'), ''):
        setattr(prefs, _field('maximize', 'type'), event.type)
        setattr(prefs, _field('maximize', 'value'), event.value or 'PRESS')
        setattr(prefs, _field('maximize', 'ctrl'), bool(event.ctrl))
        setattr(prefs, _field('maximize', 'shift'), bool(event.shift))
        setattr(prefs, _field('maximize', 'alt'), True)
        setattr(prefs, _field('maximize', 'oskey'), bool(event.oskey))
        rebuild_shortcut('maximize', context)
