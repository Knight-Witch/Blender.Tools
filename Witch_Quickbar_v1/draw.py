...
def draw_icon_rect(name, cx, cy, w, h, enabled=True):
    x = cx - w / 2
    y = cy - h / 2
    ok = draw_png_icon(name, x, y, w, h)
    if ok and not enabled:
        draw_rounded_rect(x, y, w, h, COL_ICON_DIM_OVERLAY, 2)
    if not ok:
        draw_text('?', cx, cy, int(min(w, h) * 0.55), COL_TEXT_DIM if not enabled else COL_TEXT)
...
def draw_button(button, hovered, context, runtime):
    ...
    if style not in {'bare', 'title_icon'}:
        if style == 'launcher':
            if not enabled:
                ...
                color = (0.16, 0.26, 0.46, 1.0)
                text_color = COL_TEXT
            elif hovered:
                color = (0.17, 0.17, 0.17, 0.99)
                text_color = COL_TEXT
            else:
    ...
    if button.get('icon') == 'chevron':
        ...
        icon_size = min(h * 0.86, 25 * scale)
        icon_x = x + int(19 * scale)
        draw_icon(button.get('icon', ''), icon_x, y + h / 2, icon_size, enabled)
        draw_text(button.get('text', ''), x + w / 2 + int(13 * scale), y + h / 2 + 0.5, int(14 * scale), text_color)
    elif 'icon' in button:
...
def draw_overlay(context, runtime):
    ...
    for row in layout['rows']:
        ...
        elif row.get('box') == 'footer_shortcut':
            draw_text(row.get('label', ''), row['x'] + 5, row['y'] + row['h'] / 2 + 0.5, int(11 * layout['scale']), COL_TEXT, align='LEFT')
            draw_text(row.get('value', ''), row['x'] + row['w'] * 0.48, row['y'] + row['h'] / 2 + 0.5, int(10 * layout['scale']), COL_TEXT_DIM, align='CENTER')

        if row.get('title'):
            if row.get('title_pos') == 'left':
                draw_text(row['title'], row['x'], row['y'] + row['h'] / 2 + 0.5, int(12 * layout['scale']), COL_TEXT, align='LEFT')
            else:
                draw_text(row['title'], row['x'] + row['w'] / 2, row['y'] + row['h'] / 2 + 0.5, int(13 * layout['scale']), COL_TEXT, align='CENTER')
            if row.get('box') == 'tool_title':
                grip_w = int(22 * layout['scale'])
                grip_h = int(12 * layout['scale'])
                draw_icon_rect('grip', row['x'] + row['w'] - int(16 * layout['scale']), row['y'] + row['h'] / 2, grip_w, grip_h, True)
