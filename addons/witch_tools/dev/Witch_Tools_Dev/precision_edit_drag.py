...
def materialize_edge_point(bm, obj, edge, point_world, tolerance=1.0e-6):
    point_local = obj.matrix_world.inverted_safe() @ Vector(point_world)
    candidates = []
    if edge is not None and getattr(edge, 'is_valid', False):
        candidates.append(edge)
        for vert in edge.verts:
            for linked in vert.link_edges:
                if linked.is_valid and linked not in candidates:
                    candidates.append(linked)
    if not candidates:
        candidates = [candidate for candidate in bm.edges if candidate.is_valid and not candidate.hide]

    best = None
    for candidate in candidates:
        start, end = candidate.verts
        closest, factor, distance = closest_point_segment_3d(point_local, start.co, end.co)
        if best is None or distance < best[0]:
    ...
    start, end = candidate.verts
    if (closest - start.co).length <= tolerance:
        return start
    if (closest - end.co).length <= tolerance:
        return end
...
def weld_contacts(bm, obj, contacts, face_target=None, tolerance=1.0e-6):
    target_map = {}
    ...
    for created_vert, kind, element, point_world in contacts or ():
        ...
        if kind == 'VERT':
            ...
            target_vert = materialize_edge_point(bm, obj, element, point_world, tolerance=tolerance)
        else:
            continue
