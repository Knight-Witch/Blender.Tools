from .operators_vertex_locks import _active_mesh
from .ui_helpers import draw_fullwidth_operator, draw_inline_label_prop, draw_section_toggle, draw_shortcut_hint


def draw_vertex_snap(layout, context, _props, ui):
    snap = layout.box()
    if draw_section_toggle(snap, ui, 'show_edit_vertex_snap', 'Vertex Snap', section_icon='SNAP_VERTEX', help_topic='vertex_snap_title'):
        draw_fullwidth_operator(snap, 'mesh.vertex_snap_global', 'Snap Vertices')
        draw_shortcut_hint(snap, 'mesh.vertex_snap_global')


def draw_object_snap(layout, context, props, ui):
    object_snap = layout.box()
    if not draw_section_toggle(object_snap, ui, 'show_edit_object_snap', 'Object Snap', section_icon='AREA_JOIN_DOWN', help_topic='object_snap_title'):
        return
    ...
    if select_mode[1] or select_mode[2]:
        ...
        options.prop(props, 'object_snap_match_orientation', text='Match Orientation', icon='SNAP_NORMAL', toggle=True)
        direction = options.row(align=True)
        direction.ui_units_x = 2.2
        direction.enabled = props.object_snap_match_orientation
        direction.prop_enum(props, 'object_snap_normal_mode', 'OPPOSING', text='', icon='ARROW_LEFTRIGHT')
        direction.prop_enum(props, 'object_snap_normal_mode', 'SAME', text='', icon='FORWARD')


def _draw_missing_inject(parent, context, props, ui):
    inject = parent.box()
    if not draw_section_toggle(
        inject,
        ui,
        'show_edge_doctor_inject',
    ...
    if context.mode != 'EDIT_MESH':
        ...
        inject.label(text='A/B/C: click A, B, C individually; C active last.')
        inject.label(text='L Repair: select two edges sharing one corner.')
    ...
    inject.operator('mesh.wt_edge_doctor_missing_inject', text='Repair Missing Vertex / Edge', icon='MOD_EDGESPLIT')


def _draw_curvature_sync(parent, context, props, ui):
    curvature = parent.box()
    if not draw_section_toggle(curvature, ui, 'show_edge_doctor_curvature', 'Curvature Sync', section_icon='CURVE_BEZCURVE', help_topic='curvature_sync_title'):
        return
    if context.mode != 'EDIT_MESH':
...
def _draw_alignment_fixer(parent, context, props, ui):
    align = parent.box()
    if not draw_section_toggle(align, ui, 'show_edge_doctor_alignment', 'Alignment Fixer', section_icon='PIVOT_ACTIVE', help_topic='alignment_fixer_title'):
        return
    if context.mode != 'EDIT_MESH':
    ...
    step1.operator('mesh.wt_guided_align_capture_anchor', text='Capture Selected Anchor', icon='RESTRICT_SELECT_OFF')
    captured = step1.row(align=True)
    captured.scale_y = 0.72
    captured.label(text=f'Captured: {props.align_anchor_count} verts' + (f' ({props.align_anchor_active_count} active)' if props.align_anchor_active_count else ''))
    ...
    if props.align_frame == 'CUSTOM':
        ...
        guide_status.label(text='Guide: ' + ('Start set' if props.align_guide_start_set else 'Start missing') + ' / ' + ('End set' if props.align_guide_end_set else 'End missing'))
    ...
    if needs_rails:
        step3.operator('mesh.wt_guided_align_capture_rails', text='Capture Selected Slide Rails', icon='EDGESEL')
...
def draw_edge_doctor(layout, context, props, ui):
    doctor = layout.box()
    if not draw_section_toggle(doctor, ui, 'show_edit_edge_doctor', 'Edge Doctor', section_icon='TOOL_SETTINGS', help_topic='edge_doctor_title'):
        return
    _draw_missing_inject(doctor, context, props, ui)
    _draw_alignment_fixer(doctor, context, props, ui)
    _draw_curvature_sync(doctor, context, props, ui)


def draw_selection_slots(layout, context, props, ui):
    ...
    arrow.prop(ui, 'show_edit_selection_slots', text='', emboss=False, icon='TRIA_DOWN' if slots_open else 'TRIA_RIGHT')
    slot_left.prop(ui, 'show_edit_selection_slots', text='Selection Slots', emboss=False, icon='LONGDISPLAY')
...
def draw_vertex_locks(layout, context, _props, ui):
    locks = layout.box()
    if not draw_section_toggle(locks, ui, 'show_edit_vertex_locks', 'Vertex Lock', section_icon='LOCKED', help_topic='vertex_locks_title'):
        return
    ...
    if draw_section_toggle(core, ui, 'show_vertex_locks_core', 'Vertex Locks', section_icon='FAKE_USER_ON', help_topic='vertex_locks_core_title'):
        ...
        guard.operator('witch_vertex_locks.toggle_guard', text='Guard ON' if wm.wvl_guard_enabled else 'Guard OFF', icon='FAKE_USER_ON' if wm.wvl_guard_enabled else 'FAKE_USER_OFF', depress=wm.wvl_guard_enabled)
    ...
    if draw_section_toggle(groups, ui, 'show_vertex_locks_groups', 'Groups', section_icon='GROUP_VERTEX', help_topic='vertex_locks_groups_title'):
        ...
        row.operator('witch_vertex_locks.toggle_selected', text='Lock Selected', icon='LOCKED')
