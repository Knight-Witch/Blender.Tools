...
Capture one source vertex, select a wall, enable X, choose Match Coordinates, apply, verify world X equality and unchanged Y/Z, then undo/redo. Repeat with edge, face, mixed references, and other axis combinations.
...
- Open/close, auto-start, all Align controls and tooltips.
...
- File-load recovery and unregister/re-register.
- Existing Select and Edit tools remain functional.

## Acceptance record

Record exact Blender version, add-on versions, geometry setup, measurements/screenshots for Figures A and B, undo/redo, passes/failures, and compatibility limitations.
